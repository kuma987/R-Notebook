...
## 모델링
```{r}
model <- rpart(diabetes~., data=train)
rpart # node) 분리기준, 총 개체 수, y로 예측하지 않은 개체 수, y로 예측한 개체수 (y의 실제 비율)
prp(model, type=2, extra=2, digits=3)
model$cptable #교차 타당성 오차 확인 (cp가 가장 적게 해주는 분할 수가 좋음)
plotcp(model) #교차 타당성 시각적 확인
```

## 예측 모델 생성
```{r}
pred_prob <- predict(model, newdata=test)[,2] #이진분류라서 인덱스를 통해 두 번째 클래스에 속할 확률로 지정
...
## 모델링
```{r}
#costs 인자로 오차 가중치 행렬을 지정할 수 있음. 오차 가중치가 높다 = 발생 확률을 낮춤

#costs 행렬 구조 : c(TP, FP,
#                    FN, TN)
model <- C5.0(train[,!names(train) %in% 'diabetes'], train$diabetes, trials=10)
summary(model)
plot(model)
```
